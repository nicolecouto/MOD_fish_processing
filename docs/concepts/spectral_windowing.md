...
- A P-only external CTD (DeepSolo) simply never gets `T_f`/`C_f` - `ctd.T`/`.C` are absent, so those two channels are skipped the same way an epsi channel absent from `scan.epsi` is skipped.
