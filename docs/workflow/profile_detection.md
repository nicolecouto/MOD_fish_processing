...
### `modProcess_extract_profile.m` (`processing/L1/`), orchestrated by `MODprocess_all_extract_profiles.m` (`processing/L1/`)
...
## `Profile####.mat` output: two directories, one per phase
...
**Phase A refactor (branch `epsilon_processing`):** `MODprocess_single_L1_to_L2_profile.m` used to call `modProcess_extract_profile.m` itself, doing its own file I/O and requiring a `TimeIndex` argument - it couldn't accept "any type of L1 file, whether or not already broken into a profile" the way `MODprocess_single_L1_to_L2.m`'s realtime mode already could. Extraction is now its own explicit, disk-persisted phase (`MODprocess_all_extract_profiles.m`, new), writing `profiles_raw/Profile####.mat`; `MODprocess_single_L1_to_L2_profile.m`'s signature dropped `TimeIndex` and now takes an already-extracted `profile_data` struct directly; `MODprocess_all_L1_to_L2_profiles.m` now calls the extraction orchestrator first, then loads and converts each raw file in a second loop, rather than doing both in one pass. Driven by the need to build an epsilon module (PLAN.md Section 6.4) reusing the same conversion entry point regardless of how its input data was assembled.
