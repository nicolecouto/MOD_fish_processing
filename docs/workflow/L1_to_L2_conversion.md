...
This first cut computes **raw, uncorrected power spectra only** - no epsilon, no chi, no coherence. Epsilon is a natural next step (shear `Sv` calibration is already resolved in `metadata.AFE`), but is deliberately deferred to keep this step reviewable on its own. Chi is blocked regardless: FPO7 `dTdV` needs real, time-aligned CTD temperature to fit in-situ, and DeepSolo's fallrise file has no T at all.
...
4. Assembles output arrays over the scan dimension: `dnum`, `pressure` (interpolated from this file's own `data.ctd.P` at scan center), `f` (shared, `1 × nfreq`), per-channel `P.(channel)` matrices (`nbscan × nfreq`), plus provenance (`nfft`, `dof`, `Fs_epsi`, `N_epsi`, `scan_step`).
...
- **No epsilon, chi, or SOM transfer-function correction yet** - spectra are raw and uncorrected. See PLAN.md Section 6.2/6.4 for what's still open.
